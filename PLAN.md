...
**Files:**
- `docker-compose.prod.yml` — standalone compose file (not a `-f`-merged overlay, to avoid Compose's list-merge semantics for `ports:`), `api` (no published ports) + `frontend` (`target: prod`, published to `127.0.0.1:3001` only — reachable from nginx on the same host, never directly from the internet).
- `.env.prod.example` — documents `SCRAPE_SECRET` (generate with `openssl rand -hex 24`); real `.env.prod` is gitignored.
- `deploy/nginx-outlet-monitor.conf` — the third `server` block (reference copy) added to the server's existing `/etc/nginx/sites-available/murilopreto.com.br`, listening on `7716 ssl`, reusing the existing cert, `proxy_pass http://127.0.0.1:3001`.

**Port note:** the server turned out to already be running a small fleet of other Docker services (a separate portfolio site's `portfolio-flask` container on host port 3000, plus Sonarr/Radarr/Bazarr/Prowlarr/Plex/Pi-hole/Portainer/etc. on their own ports) — `127.0.0.1:3000` for this app's frontend collided with the already-running `portfolio-flask` container, so the internal loopback port was moved to `3001` instead. Also worth noting for future reference: the internal loopback port can never be the *same* number as the public port nginx listens on (`7716` here) — `listen 7716 ssl;` binds the wildcard address, which already covers `127.0.0.1:7716`, so Docker can't separately bind that host:port too. The two ports are unrelated by design; only nginx's `proxy_pass` target needs to match whatever internal port `docker-compose.prod.yml` publishes to.
